...
class Post {
            ...
            return new Promise((res, rej) => {
                form(
                    poster.url,
                    {
                        file: file,
                        field: 'files',
                        filepath: this.getServerpath(file, poster)
                    },
                    (err, resp, msg) => {
                        if(err){
                            rej(err);
                            ...
                            resp.statusCode === 200 ?
                                ...
                                console.log(color.yellow('Wran: ' + msg));
                        }
                    }
                )
            })
    ...
    getServerpath(file, poster) {
        var serverpath, local, cwd;
        // 获取服务器路径配置
        serverpath = poster.paths.find((url) => {
            // 标准化路径
        ...
        return path.join(serverpath.server, file);
    }
