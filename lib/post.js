...
class Post {
    ...
    // 创建配置文件
    create(src) {
        lib.isFile(src) || fs.writeFileSync(src, '{}', 'utf-8');
    }
    ...
    post() {
        ...
        var poster = config.poster[config.posterSelectedIndex || 0];

        // 当前CONFIG不存在paths配置时
        if(!config.poster[config.posterSelectedIndex || 0].paths){
                ...
                form(
                    ...
                    (err, resp, msg) => {
                        if(err){
                            ...
                            res(msg);
                            resp.statusCode === 200 ?
                                console.log(color.cyan('=> ' + poster.name), paths.serverFilepath):
                                console.log(color.yellow('Warn: ' + msg), color.gray('[post|post.js|97]'));
                        }
                    }
                )
            })
